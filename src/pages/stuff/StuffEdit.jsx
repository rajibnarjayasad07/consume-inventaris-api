...
export default function StuffEdit() {
    ...
    const [error, setError] = useState({}); // Ubah inisialisasi state error menjadi objek kosong
    ...
    }, [id]); // Tambahkan id sebagai dependency agar efek hanya terjadi saat id berubah
    ...
    const handleEditStuff = (event) => {
        ...
        .catch(err => {
            setError(err.response.data); // Ubah setError dengan data keseluruhan respons
        ...
        <Case>
            <div className="block m-auto h-screen bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <div className="items-center m-5 pb-10 pt-10">
                    {error && Object.keys(error).length > 0 && ( // Perbaiki pengecekan kondisi error
                            ...
                            <div className="border border-t-0 border-red-400 rounded-b bg-red-100 px-4 py-3 text-red-700">
                                <ul>
                                    {Object.entries(error).map(([key, value], i) => (
                                        <li key={key}>{key !== "status" ? i+1 + '. ' + value : ''}</li> // Perbaiki perbandingan operator tidak sama
                                    ))}
                                </ul>
                            </div>
                        </div>
                    ...
                    <form className="max-w-sm mx-auto" onSubmit={handleEditStuff}>
                        <div className="mb-5">
                            ...
                            placeholder="Ketik Nama Barang" required value={forms.name} onChange={e => setForms({...forms, name: e.target.value})}  /> {/* Gunakan value untuk menghubungkan input dengan state */}
                        </div>
                        <div className="mb-5">
                            <label htmlFor="category" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kategori Barang</label>
                            <select id="category" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                            focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 
                            dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={forms.category} onChange={e => setForms({...forms, category: e.target.value})}>
                                <option disabled>Pilih Kategori</option> {/* Tambahkan atribut disabled untuk opsi default */}
